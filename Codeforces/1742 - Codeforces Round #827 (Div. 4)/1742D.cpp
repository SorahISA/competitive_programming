...
void solve() {
    ...
    for (int i = 1, a; i <= N; ++i) cin >> a, pos[a].eb(i);
    
    int ans = -1;
    if (SZ(pos[1]) >= 2) chmax(ans, end(pos[1])[-1] + end(pos[1])[-2]);
    for (int c = 1; c < maxc; ++c) for (int d = 1; d < maxc; ++d) if (gcd(c, d) == 1) {
        if (SZ(pos[c]) and SZ(pos[d])) chmax(ans, end(pos[c])[-1] + end(pos[d])[-1]);
    }
    print(ans);
}
