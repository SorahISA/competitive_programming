...
void solve() {
    ...
    for (int q = 0; q < Q; ++q) {
        ...
        for (char c : X) (d == 1 ? cntS : cntT)[c - 'a'] += k;
        for (int i = 1; i < 26; ++i) {
            if (cntS[i]) S_has_non_a = true;
            if (cntT[i]) T_has_non_a = true;
        }
        
        if (T_has_non_a) print("Yes");
        else if (S_has_non_a) print("No");
        else print(cntS[0] >= cntT[0] ? "No" : "Yes");
    }
}
