...
void solve() {
    ...
    map<string, int> cnt;
    for (string &s : v1) cin >> s, ++cnt[s];
    for (string &s : v2) cin >> s, ++cnt[s];
    for (string &s : v3) cin >> s, ++cnt[s];
    
    int s1 = 0, s2 = 0, s3 = 0;
    for (int i = 0; i < N; ++i) {
        s1 += (cnt[v1[i]] == 1 ? 3 : (cnt[v1[i]] == 2 ? 1 : 0));
        s2 += (cnt[v2[i]] == 1 ? 3 : (cnt[v2[i]] == 2 ? 1 : 0));
        s3 += (cnt[v3[i]] == 1 ? 3 : (cnt[v3[i]] == 2 ? 1 : 0));
    }
    print(s1, s2, s3);
}
