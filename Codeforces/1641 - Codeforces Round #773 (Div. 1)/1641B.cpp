...
void solve() {
    ...
    for (int i = N-1; i >= 0; i -= 2) {
        int flag = -1;
        for (int j = i-1; j >= 0; --j) {
            if (A[j] == A[i]) {flag = j; break;}
        }
        debug(A[i], flag);
        if (flag == i-1) {ans.eb(2); continue;}
        for (int j = i-1, pos = flag; j >= flag; --j, ++pos) query.eb(pos, A[j]);
        ans.eb(2*(i-flag)), ans.eb(2);
        reverse(begin(A) + flag, begin(A) + i);
        // debug(A, query, ans);
    }
    reverse(ALL(ans));
    
    cout << SZ(query) << "\n";
    for (auto &[pos, val] : query) cout << pos << " " << val << "\n";
    cout << SZ(ans) << "\n";
