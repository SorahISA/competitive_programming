...
void solve() {
    ...
    for (auto [a, b] : rect) {
        vector<int> cost{0}; cost.reserve(a+b+1);
        for (int i = a; i > b; --i) cost.eb(b);
        for (int i = b; i > 1; --i) cost.eb(i), cost.eb(i-1);
        cost.eb(1), cost.eb(0);
        partial_sum(ALL(cost), begin(cost));
        // debug(a, b, cost);
        
        vector<int> tmp = dp;
        for (int c = 0; c <= a+b; ++c) {
            for (int k = K; k >= c; --k) chmin(tmp[k], dp[k-c] + cost[c]);
        }
        dp = tmp;
    }
