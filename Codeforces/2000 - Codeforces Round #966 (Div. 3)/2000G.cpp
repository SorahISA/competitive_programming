...
void solve() {
    ...
    for (int i = 0; i < M; ++i) {
        ...
        adj[u].eb(v, l1, l2), adj[v].eb(u, l1, l2);
    }
    
    vector<int> dis(N+1, -1);
    {
        Prior<pii> pq;
        dis[N] = t0, pq.ee(dis[N], N);
        while (SZ(pq)) {
            auto [d, now] = pq.top(); pq.pop();
            if (d != dis[now]) continue;
            for (auto [x, l1, l2] : adj[now]) {
                if ((t1 < d and d < t2) or (t1 < d-l1 and d-l1 < t2) or (d >= t2 and d-l1 <= t1)) {
                    if (chmax(dis[x], max(t1 - l1, d - l2))) pq.ee(dis[x], x);
                }
                else if (chmax(dis[x], d - l1)) pq.ee(dis[x], x);
            }
        }
    }
