...
void solve() {
    ...
    vector<int> id(N);
    iota(ALL(id), 0), sort(ALL(id), [&](int x, int y) { return deg[x] > deg[y]; });
    
    auto chk = [&](int x, int y) -> int {
        int res = deg[x] + deg[y] - 1;
        for (auto [u, v] : edges) {
            if (minmax(u, v) == minmax(x, y)) res -= 1;
        }
        return res;
    };
    
    int ans = max({chk(id[0], id[1]), chk(id[0], id[2]), chk(id[1], id[2])});
    
    vector<int> is_adj_to_id0(N, 0);
    for (auto [u, v] : edges) {
        if (u == id[0]) is_adj_to_id0[v] = 1;
        if (v == id[0]) is_adj_to_id0[u] = 1;
    }
    for (int i = 0; i < N; ++i) {
        if (i != id[0]) chmax(ans, deg[id[0]] + deg[i] - 1 - is_adj_to_id0[i]);
    }
