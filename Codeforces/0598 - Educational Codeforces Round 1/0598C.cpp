...
void solve() {
    ...
    sort(ALL(id), [&](int i, int j) {
        if (quad(pts[i]) != quad(pts[j])) return quad(pts[i]) < quad(pts[j]);
    ...
    pii ans = {0, 0};
    __float128 min_angle = 2 * PI;
    for (int i = 0; i < N; ++i) {
        /// find angle pts[id[i]], pts[id[i+1]] ///
        __float128 a0 = atan2l(pts[id[i  ]].Y, pts[id[i  ]].X);
        __float128 a1 = atan2l(pts[id[i+1]].Y, pts[id[i+1]].X);
        __float128 angle = a1 - a0;
        if (angle < 0) angle += 2 * PI;
        if (angle > PI) angle = 2 * PI - angle;
        if (chmin(min_angle, angle)) ans = {id[i], id[i+1]};
    }
