...
vector<int> adj[maxn], jda[maxn], rnk, dis, vis;
...
void dijkstra(int st) {
    ...
    while (SZ(pq)) {
        ...
        vis[now] = 1;
        for (int x : jda[now]) {
            if (chmin(dis[x], d + rnk[x])) pq.ee(dis[x], x);
            --rnk[x];
        }
    }
}
...
void solve() {
    ...
    for (int i = 0, u, v; i < M; ++i) cin >> u >> v, adj[u].eb(v), jda[v].eb(u);
    
    rnk.resize(N+1);
    for (int i = 1; i <= N; ++i) rnk[i] = SZ(adj[i]);
