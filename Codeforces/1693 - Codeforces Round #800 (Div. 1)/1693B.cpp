...
void solve() {
    ...
    int cnt = 0;
    vector<int> val(N, 0), lazy(N, 0);
    for (int i = N-1; i >= 0; --i) {
        if (val[i] < rang[i].X) lazy[i] += rang[i].Y - val[i], val[i] = rang[i].Y, ++cnt;
        if (lazy[i]) {
            int tmp = min(val[p[i]] + lazy[i], rang[p[i]].Y) - val[p[i]];
            val[p[i]] += tmp, lazy[p[i]] += tmp;
            // debug(i, p[i], lazy[i], val[p[i]]);
        }
    }
    cout << cnt << "\n";
}
