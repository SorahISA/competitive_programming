...
void solve() {
    ...
    /// A[x] = max{ A[x+1]-1, 0 } in each iteration ///
    
    int ans = 1;
    for (int i = 0; i < K; ++i) {
        /// information unknown => A[i] \in [0, i] => (i+1) possiblities ///
        ans = ans * (i+1) % mod;
    }
    for (int i = 0; i+K < N; ++i) {
        if (A[i] > 0) {
            /// then A[i+K] = A[i] + K > 0 => only 1 possiblity ///
        }
        else if (A[i] == 0) {
            /// then A[i+K] \in [0, K] => (K+1) possiblities ///
            ans = ans * (K+1) % mod;
        }
        else if (A[i] == -1) {
            /// then A[i] \in [0, i] => A[i+K] \in [0, i+K] => min{ i+K+1, N } possiblities ///
            ans = ans * min(i+K+1, N) % mod;
        }
    }
    for (int i = N-K; i < N; ++i) {
        if (A[i] > 0) {
            /// not possible, after K iterations the last K numbers must be sorted ///
            ans = 0;
        }
    }
    cout << ans << "\n";
