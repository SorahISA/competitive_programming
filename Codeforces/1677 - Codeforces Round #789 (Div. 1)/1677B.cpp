...
void solve() {
    ...
    string T = "";
    for (int i = 0; i < M; ++i) {
        for (int j = i; j < N*M; j += M) T += S[j];
    }
    
    vector<int> pre_S(N*M), pre_T(N*M);
    for (int i = 0; i < N*M; ++i) pre_S[i] = S[i] == '1';
    for (int i = 0; i < N*M; ++i) pre_T[i] = T[i] == '1';
    partial_sum(ALL(pre_S), begin(pre_S));
    for (int i = 0; i < N*M; i += N) partial_sum(begin(pre_T) + i, begin(pre_T) + i + N, begin(pre_T) + i);
    
    vector<int> row(N * M), col(N * M);
    for (int j = 0; j < M; ++j) {
        row[j] = (j ? row[j-1] : 0) | (S[j] == '1');
        col[j] = (j ? col[j-1] : 0) + (S[j] == '1');
    }
    for (int i = 1; i < N; ++i) {
        for (int j = 0; j < M; ++j) {
            row[i*M+j] = row[(i-1)*M+j] + (pre_S[i*M+j] - pre_S[(i-1)*M+j] > 0);
            col[i*M+j] = col[i*M+(j-1)] + (pre_T[j*N+i] == 1 and S[i*M+j] == '1');
        }
    }
