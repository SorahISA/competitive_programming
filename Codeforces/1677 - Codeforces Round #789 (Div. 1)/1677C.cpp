...
void solve() {
    ...
    vector<int> A(N), B(N), adj(N);
    for (int &x : A) cin >> x, --x;
    for (int &x : B) cin >> x, --x;
    for (int i = 0; i < N; ++i) adj[A[i]] = B[i];
    
    vector<int> cycle_sz, vis(N, 0);
    for (int i = 0; i < N; ++i) {
        int now = i, sz = 0;
        while (!vis[now]) vis[now] = 1, now = adj[now], ++sz;
        if (sz & ~1) cycle_sz.eb(sz & ~1);
    }
    // debug(cycle_sz);
    ...
    for (int &sz : cycle_sz) {
        vector<int> nums;
        for (int i = 0; i < sz/2; ++i) nums.eb(mn++), nums.eb(mx--);
        for (int i = 0; i < sz; ++i) ans += abs(nums[i] - nums[(i+1)%sz]);
        // debug(nums);
    }
    cout << ans << "\n";
}
