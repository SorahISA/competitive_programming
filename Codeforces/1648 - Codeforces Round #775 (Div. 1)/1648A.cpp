...
void solve() {
    ...
    vector<int> row[maxc], col[maxc];
    for (int i = 0; i < N; ++i) {
        for (int j = 0, c; j < M; ++j) cin >> c, --c, row[c].eb(i), col[c].eb(j);
    }
    ...
    for (int i = 0; i < maxc; ++i) {
        ...
        for (int j = 1; j < SZ(row[i]); ++j) {
            ans += (row[i][j] - row[i][j-1]) * j * (SZ(row[i]) - j);
        }
        // debug(i, "row"s, ans, row[i]);
        for (int j = 1; j < SZ(col[i]); ++j) {
            ans += (col[i][j] - col[i][j-1]) * j * (SZ(col[i]) - j);
        }
        // debug(i, "col"s, ans, col[i]);
    }
    cout << ans << "\n";
}
