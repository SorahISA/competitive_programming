...
struct IntervalContainer {
    ...
    void add(int l, int r, T v) {
        ...
        auto it_L = itv.lower_bound({l, l});
        auto it_R = itv.lower_bound({r+1, r+1});
        while (it_L != it_R) it_L->second += v, it_L = next(it_L);
    }
...
int fpow(int base, int exp, int _mod = mod, int ans = 1) {
    while (exp) {
        if (exp & 1) ans = ans * base % _mod;
        exp >>= 1, base = base * base % _mod;
    }
    return ans % _mod;
}
...
void solve() {
    ...
    for (int i = 1; i <= N; ++i) itv.modify(i, i, rnd() % vmax + 1);
    // debug(itv.get(1, N));
    ...
    for (int q = 1; q <= M; ++q) {
        ...
        if (op == 1) itv.add(l, r, x);
        if (op == 2) itv.modify(l, r, x);
        if (op == 3) {
            auto ret = itv.get(l, r);
            // debug(ret);
            sort(ALL(ret), [&](const auto &a, const auto &b) { return a.second < b.second; });
            for (int now = 0; auto [lr, v] : ret) {
                if ((now += lr.second - lr.first + 1) >= x) { print(v); break; }
            }
        }
        if (op == 4) {
            auto ret = itv.get(l, r);
            // debug(ret);
            int sum = 0;
            for (auto [lr, v] : ret) sum += (lr.second - lr.first + 1) * fpow(v % y, x, y);
            print(sum % y);
        }
