...
void solve() {
    ...
    function<void(int)> dfs = [&](int now) {
        if (lc[now] != -1) dfs(lc[now]);
        if (rc[now] != -1) dfs(rc[now]);
        for (int c = 0; c < M; ++c) {
            int way_l = (lc[now] != -1 ? (c ? dp[lc[now]][c-1] : 0) : 1);
            int way_r = (rc[now] != -1 ? dp[rc[now]][c] : 1);
            dp[now][c] = ((c ? dp[now][c-1] : 0) + way_l * way_r) % mod;
        }
        // debug(now, dp[now]);
    };
    dfs(root);
