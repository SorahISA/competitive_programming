...
void solve() {
    ...
    int lgN = __lg(N+1) + 2;
    vector jmp(lgN, vector<int>(N)); jmp[0] = R;
    for (int lay = 1; lay < lgN; ++lay) {
        for (int i = 0; i < N; ++i) jmp[lay][i] = jmp[lay-1][jmp[lay-1][i]];
    }
    
    vector<int> app(N, 0);
    for (int i = 0; i < N; ++i) app[jmp[lgN-1][i]] = 1;
    
    if (count(ALL(app), 1) == N) { print(2); return; }
    
    int ans = 0;
    vector<int> cur(N); iota(ALL(cur), 0);
    for (int lay = lgN-1; lay >= 0; --lay) {
        bool flag = false;
        for (int i = 0; i < N; ++i) {
            if (not app[jmp[lay][cur[i]]]) flag = true;
        }
        if (flag) {
            for (int i = 0; i < N; ++i) cur[i] = jmp[lay][cur[i]];
            ans += 1 << lay;
            // debug(lay, ans, cur);
        }
    }
