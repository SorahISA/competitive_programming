...
void solve() {
    ...
    int tok = 0;
    for (int x : A) { if (++cnt[x] == 1) B[tok++] = x; }
    for (int i = 1; i <= N; ++i) { if (not cnt[i]) B[tok++] = i; }
    print(B);
    
    // vector<int> B{A[0]}, mx{1};
    // vector<int> cnt(N+1, 0); ++cnt[A[0]];
    // for (int i = 1; i < N; ++i) {
    //     while (SZ(B) and cnt[A[i]] + (i+1 - SZ(B)) < mx.back()) --cnt[B.back()], B.pb(), mx.pb();
    //     while (SZ(B) <= i) B.eb(A[i]), mx.eb(max(SZ(mx) ? mx.back() : 0, ++cnt[A[i]]));
    //     // debug(i, B);
    // }
    // print(B);
}
