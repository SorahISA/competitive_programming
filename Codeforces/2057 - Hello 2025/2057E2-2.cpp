...
void solve() {
    ...
    vector answers(N, vector(N, vector<int>(N, -1)));
    vector dis(N, vector<int>(N, N));
    
    for (auto [w, u, v] : edges) dis[u][v] = dis[v][u] = 1;
    ...
    auto update_dis = [&](int a, int b, int w) {
        for (int i = 0; i < N; ++i) for (int j = 0; j < N; ++j) {
            if (
                chmin(dis[i][j], dis[i][a] + dis[b][j]) or
                chmin(dis[i][j], dis[i][b] + dis[a][j])
            ) answers[i][j][dis[i][j] + 1] = w;
        }
    };
    ...
    for (auto [k, a, b] : queries) ans.eb(answers[a][b][k]);
    print(ans);
}
