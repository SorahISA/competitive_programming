...
void solve() {
    ...
    for (int tok = 0; auto &[k, a, b, id] : queries) cin >> a >> b >> k, --a, --b, id = tok++;
    sort(ALL(queries));
    
    vector<int> ans(Q, weights.back() + 1);
    
    vector dis(N, vector<int>(N));
    auto init_dis = [&](int thres) {
        for (int i = 0; i < N; ++i) fill(ALL(dis[i]), N), dis[i][i] = 0;
        vector<int> vis(N, -1);
        deque<int> deq;
        for (int i = 0; i < N; ++i) {
            deq.eb(i);
            while (SZ(deq)) {
                ...
                for (auto [x, w] : adj[now]) {
                    if (chmin(dis[i][x], dis[i][now] + (w > thres))) {
                        if (w > thres) deq.eb(x); else deq.ef(x);
                    }
                }
            }
        }
    ...
    for (int w : weights) {
        ...
        for (auto [k, a, b, id] : queries) {
            if (dis[a][b] < k) chmin(ans[id], w);
        }
    }
    print(ans);
}
