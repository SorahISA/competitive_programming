...
void solve() {
    int A; cin >> A;
    string _B, C; cin >> _B >> C;
    
    vector<int> tr(128, 0);
    vector<char> inv(128, '\0');
    for (int i =  0, c = '0'; c <= '9'; ++i, ++c) tr[c] = i, inv[i] = c;
    for (int i = 10, c = 'A'; c <= 'Z'; ++i, ++c) tr[c] = i, inv[i] = c;
    
    int val = 0;
    for (char c : C) val = val * A + tr[c];
    ...
    if (_B == "R") {
        ...
        int B = stoi(_B);
        do ans += inv[val % B], val /= B; while (val);
        reverse(ALL(ans));
    }
    cout << ans << "\n";
}
