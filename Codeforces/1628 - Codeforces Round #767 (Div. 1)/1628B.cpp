...
const int maxc = 26*26*26 + 26*26 + 26 + 5;
...
void solve() {
    ...
    int flag = 0;
    for (string &S : vec) {
        if (S[0] == S[SZ(S)-1]) flag = 1;
    }
    
    auto trans = [](string &S) {
        int val = 0;
        for (char &c : S) val = 26 * val + (c - 'a') + 1;
        return val;
    };
    
    vector<int> cnt_l(maxc, 0), cnt_r(maxc, 0);
    for (string &S : vec) ++cnt_r[trans(S)];
    for (string &S : vec) {
        static string T;
        --cnt_r[trans(S)];
        if (SZ(S) == 2) {
