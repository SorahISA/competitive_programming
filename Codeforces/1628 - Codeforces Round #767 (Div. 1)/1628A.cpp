...
void solve() {
    ...
    vector<int> A(N), B, app(N+1, 0);
    for (int &x : A) cin >> x, app[x] = 1;
    
    int K = 0;
    while (app[K]) ++K;
    
    deque<int> num[N+1];
    for (int i = 0; i < N; ++i) num[A[i]].eb(i);
    for (int i = 0; i < K; ++i) num[i].eb(N);
    
    int mex = K, max_id = -1;
    while (mex) {
        int now = 0, flag = 0;
        for (int i = 0; i < mex; ++i) {
            if (num[i][0] < N) flag = 1, chmax(now, num[i][0]);
            else mex = i;
        }
        debug(mex, now, flag);
        ...
        if (flag) B.eb(mex);
        for (int i = 0; i < mex; ++i) {
            while (num[i].front() <= now) num[i].pf();
        }
        chmax(max_id, now);
    }
    while (++max_id < N) B.eb(0);
