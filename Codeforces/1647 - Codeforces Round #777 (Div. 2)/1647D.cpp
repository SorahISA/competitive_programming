...
int divceil(int x, int y) {return (x - 1) / y + 1;}

void solve() {
    int N, D; cin >> N >> D;
    
    if (N % (D * D)) return cout << "No\n", void();
    int d_pow = 0;
    while (N % D == 0) N /= D, ++d_pow;
    ...
    if (d_pow >= 3) {
        ...
        for (int i = 2; i*i <= D; ++i) {
            int cnt = 0;
            while (D % i == 0) D /= i, ++cnt;
            if (cnt) all_cnt.eb(i, cnt);
        }
        if (D > 1) all_cnt.eb(D, 1);
        // debug(all_cnt);
        
        if (SZ(all_cnt) >= 2) return cout << "Yes\n", void();
        
        int add_cnt = 0;
        while (N % all_cnt[0].X == 0) N /= all_cnt[0].X, ++add_cnt;
        if (all_cnt[0].Y + add_cnt <= (d_pow - 1) * (all_cnt[0].Y - 1)) return cout << "Yes\n", void();
    }
