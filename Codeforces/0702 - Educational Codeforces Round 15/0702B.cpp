...
void solve() {
    ...
    vector<pii> cnt{{A[0], 0}};
    for (int &x : A) {
        if (x == cnt.back().X) ++cnt.back().Y;
        else cnt.eb(x, 1);
    }
    N = SZ(cnt);
    
    int ans = 0;
    for (int goal = 2; goal <= INT_MAX; goal <<= 1) {
        for (int i = 0, j = N-1; i <= j; ++i) {
            while (j > i and cnt[i].X + cnt[j].X > goal) --j;
            if (cnt[i].X + cnt[j].X == goal) {
                // debug(i, j, cnt[i], cnt[j]);
                if (i == j) ans += cnt[i].Y * (cnt[i].Y - 1) / 2;
                else        ans += cnt[i].Y * cnt[j].Y;
            }
        }
    }
    cout << ans << "\n";
