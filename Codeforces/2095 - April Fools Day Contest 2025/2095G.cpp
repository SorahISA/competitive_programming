...
void solve() {
    ...
    vector<pdd> slimes(N);
    for (auto &[x, y] : slimes) cin >> x >> y;
    sort(ALL(slimes));
    
    double ans = 1e50;
    for (int i = 0, j = K-1; j < N; ++i, ++j) {
        double dist2 = (slimes[i].X - slimes[j].X) * (slimes[i].X - slimes[j].X)
                     + (slimes[i].Y - slimes[j].Y) * (slimes[i].Y - slimes[j].Y);
        double area = dist2 / 4.0 * acos(-1);
        chmin(ans, area);
    }
