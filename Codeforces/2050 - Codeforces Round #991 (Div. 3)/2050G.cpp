...
void solve() {
    ...
    int ans = *max_element(ALL(deg));
    
    auto dfs = [&](int now, int par, auto &&self) -> int {
        vector<int> ch;
        for (int x : adj[now]) { if (x != par) ch.eb(self(x, now, self)); }
        if (SZ(ch)) {
            sort(RALL(ch));
            int sum = ch[0] + (SZ(ch) >= 2 ? ch[1] : 0) + deg[now];
            chmax(ans, sum);
            return max<int>(ch[0] + deg[now] - 2, 0);
        }
        return 0;
    };
    dfs(0, 0, dfs);
