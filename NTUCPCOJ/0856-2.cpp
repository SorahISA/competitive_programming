...
void solve() {
    ...
    for (int i = 1; i <= M; ++i) {
        ...
        all.eb(u * maxc + w), all.eb(v * maxc + w);
    }
    
    vector<vector<int>> tickets(N+1);
    for (int u = 1; u <= N; ++u) {
        int K, a; cin >> K, tickets[u].resize(K);
        all.eb(u);
        while (K--) cin >> a, tickets[u][K] = a, all.eb(u * maxc + a);
    }
    
    make_unique(all);
    
    DSU dsu; dsu.init(all);
    for (int u = 1; u <= N; ++u) {
        for (auto [v, w] : adj[u]) dsu.U(u * maxc + w, v * maxc + w);
        for (int a : tickets[u]) dsu.U(u, u * maxc + a);
    }
    ...
    for (int t = 1; t <= N; ++t) {
        int n_op = 0;
        for (auto [x, w] : adj[t]) n_op += dsu.U(t, t * maxc + w);
        for (auto [s, l, q] : queries[t]) ans[q] = (s == t or (binary_search(ALL(all), s * maxc + l) and dsu.R(s * maxc + l) == dsu.R(t)));
        while (n_op--) dsu.undo();
    }
    for (int x : ans) print(x ? "Yes" : "No");
}
