...
void solve() {
    ...
    int sumA = accumulate(ALL(A), int(0)), sumB = accumulate(ALL(B), int(0));
    int ans = max(sumA, sumB), l = -2, r = -2;
    for (int i = 0, sum = 0, lst = 0; i < N; ++i) {
        sum += A[i] - B[i];
        if (sum < 0) sum = 0, lst = i + 1;
        if (chmax(ans, sum + sumB)) l = lst, r = i;
    }
    for (int i = 0, sum = 0, lst = 0; i < N; ++i) {
        sum += B[i] - A[i];
        if (sum < 0) sum = 0, lst = i + 1;
        if (chmax(ans, sum + sumA)) l = lst, r = i;
    }
    print(ans, l+1, r+1);
}
