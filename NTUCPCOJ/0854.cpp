...
void solve() {
    int N; double Sx, Sy; cin >> N >> Sx >> Sy;
    
    vector<pair<double, int>> cnt;
    for (int i = 1; i <= N; ++i) {
        int _x1, _x2, _y; cin >> _x1 >> _x2 >> _y;
        double x1(_x1), x2(_x2), y(_y);
        /// Sy -> y  -> 0 ///
        /// Sx -> x1 -> ? ///
        /// Sx -> x2 -> ? ///
        x1 = Sx + (x1 - Sx) * Sy / (Sy - y);
        x2 = Sx + (x2 - Sx) * Sy / (Sy - y);
        cnt.eb(x1, 1), cnt.eb(x2, -1);
    }
    sort(ALL(cnt));
    
    double ans = 0.0, lst = -1e20; int sum = 0;
    for (auto [key, val] : cnt) {
        if (sum > 0) ans += key - lst;
        sum += val, lst = key;
    }
    cout << fixed << setprecision(15), print(ans);
}
