...
struct DSU {
    ...
    int R(int x) { if (not p.contains(x)) return sz[x] = 1, p[x] = x; return _R(x); }
...
void solve() {
    ...
    for (int i = 1; i <= M; ++i) {
        ...
        dsu.U(u * maxc + w, v * maxc + w);
    }
    
    for (int u = 1; u <= N; ++u) {
        int K, a; cin >> K;
        while (K--) cin >> a, dsu.U(u, u * maxc + a);
    }
    ...
    for (int t = 1; t <= N; ++t) {
        ...
        for (auto [s, l, q] : queries[t]) ans[q] = (s == t or dsu.R(s * maxc + l) == dsu.R(t));
        while (n_op--) dsu.undo();
    }
