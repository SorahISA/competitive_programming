...
template <typename T> struct M {
    ...
    friend M &operator/=(M &a, M b) { return a = a / b; }
    friend ostream &operator<<(ostream &os, const M &b) { return os << b.v; }
};
using mint = M<int>;

const int mod = 998'244'353;
template <> int mint::MOD = mod;

void solve() {
    int N, K; cin >> N >> K;
    const mint invK = mint(K).inv();
    
    valarray<size_t> A(N), jmp(N), tmpu(N);
    valarray<mint> B(N), ans(N), tmpi(N), zero(N);
    for (auto &x : A) cin >> x, --x;
    for (int _; auto &x : B) cin >> _, x = _;
    jmp = A;
    
    while (K) {
        if (K & 1) ans[A] += B, tmpi = B, B = zero, B[jmp] += tmpi;
        K >>= 1, B[jmp] += (tmpi = B), tmpu = jmp[jmp], jmp = tmpu;
    }
    for (mint x : ans) print(int(x * invK));
}
