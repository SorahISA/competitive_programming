...
void solve() {
    ...
    vector<int> primes;
    bitset<sqmxc> is_prime;
    is_prime.set(), is_prime[0] = is_prime[1] = 0;
    for (int i = 2; i < sqmxc; ++i) {
        if (not is_prime[i]) continue;
        primes.eb(i);
        for (int j = i*i; j < sqmxc; j += i) is_prime[j] = 0;
    }
    
    int ans = 0;
    for (int x : primes) {
        if (x*x*x*x*x*x*x*x > N) break;
        ++ans;
    }
    
    for (int i = 0, j = SZ(primes) - 1; i < j; ++i) {
        while (j > i and primes[i] * primes[i] * primes[j] * primes[j] > N) --j;
        ans += j - i;
    }
    print(ans);
}
