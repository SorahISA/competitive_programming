...
void solve() {
    ...
    auto valid = [&](int r, int c) -> bool {
        return (0 <= r and r < H and 0 <= c and c < W and grid[r][c] != '#');
    };
    
    vector dis(H, vector<int>(W, INF));
    deque<pii> que;
    for (int r = 0; r < H; ++r) for (int c = 0; c < W; ++c) {
        if (grid[r][c] == 'H') dis[r][c] = 0, que.eb(r, c);
    }
    
    while (SZ(que)) {
        auto [r, c] = que[0]; que.pf();
        for (int d : {0, 1, 2, 3}) {
            int nr = r + dx[d], nc = c + dy[d];
            if (valid(nr, nc) and chmin(dis[nr][nc], dis[r][c] + 1)) que.eb(nr, nc);
        }
    }
