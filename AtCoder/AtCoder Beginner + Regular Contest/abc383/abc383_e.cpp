...
struct DSU {
    ...
    void init(int N) {
        ...
        sa.assign(maxn, 0), sb.assign(maxn, 0);
    }
    
    int R(int x) { return x ^ p[x] ? p[x] = R(p[x]) : x; }
    int U(int x, int y) { return (x = R(x)) ^ (y = R(y)) ? sa[y] += sa[x], sb[y] += sb[x], p[x] = y, 1 : 0; }
...
void solve() {
    ...
    DSU dsu; dsu.init(N);
    for (int i = 0, a; i < K; ++i) cin >> a, --a, ++dsu.sa[a];
    for (int i = 0, b; i < K; ++i) cin >> b, --b, ++dsu.sb[b];
    
    int ans = 0;
    for (auto [w, u, v] : edges) {
        if (not dsu.U(u, v)) continue;
        u = dsu.R(u);
        int tmp = min(dsu.sa[u], dsu.sb[u]);
        dsu.sa[u] -= tmp, dsu.sb[u] -= tmp;
        ans += tmp * w;
    }
    print(ans);
}
