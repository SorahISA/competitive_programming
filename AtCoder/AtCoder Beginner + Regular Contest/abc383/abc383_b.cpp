...
void solve() {
    ...
    int ans = 0;
    for (int r1 = 0; r1 < H; ++r1) for (int c1 = 0; c1 < W; ++c1) if (grid[r1][c1] == '.') {
        for (int r2 = 0; r2 < H; ++r2) for (int c2 = 0; c2 < W; ++c2) if (grid[r2][c2] == '.') {
            int cnt = 0;
            for (int r = 0; r < H; ++r) for (int c = 0; c < W; ++c) if (grid[r][c] == '.') {
                cnt += (min(abs(r - r1) + abs(c - c1), abs(r - r2) + abs(c - c2)) <= D);
            }
            chmax(ans, cnt);
        }
    }
