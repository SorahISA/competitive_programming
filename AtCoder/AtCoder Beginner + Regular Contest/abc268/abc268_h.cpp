...
struct AC {
    ...
    void Build() {
        ...
        while (SZ(que)) {
            ...
            for (int i = 0; i < maxsigma; ++i) {
                ...
                int tmp = fail[now];
                while (tmp and !tr[tmp][i]) tmp = fail[tmp];
                fail[tr[now][i]] = tr[tmp][i];
                que.eb(tr[now][i]);
    ...
    int Search(const string &str) {
        int ans = 0;
        for (int now = 0; char c : str) {
            while (now and !tr[now][c-'a']) now = fail[now];
            now = tr[now][c-'a'];
            if (is_end[now]) ++ans, now = 0;
        }
        return ans;
    }
};
