...
void solve() {
    ...
    int sum = 0;
    prior<tuple<int, int, int>> pq; pq.ee(grid[P][Q], P, Q);
    vector vis(H, vector<int>(W, 0)); vis[P][Q] = 1;
    
    auto valid = [&](int x, int y) -> bool { return (0 <= x and x < H and 0 <= y and y < W and not vis[x][y]); };
    
    while (SZ(pq)) {
        auto [now, x, y] = pq.top(); pq.pop();
        if (sum and i128(now) * X >= sum) break;
        sum += now;
        for (int d : {0, 1, 2, 3}) {
            int nx = x+dx[d], ny = y+dy[d];
            if (valid(nx, ny)) vis[nx][ny] = 1, pq.ee(grid[nx][ny], nx, ny);
        }
    }
    print(sum);
}
