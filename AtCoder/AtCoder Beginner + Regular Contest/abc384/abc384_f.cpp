...
void solve() {
    ...
    // for (int i = 0; i < N; ++i) for (int j = i; j < N; ++j) ans += A[i] + A[j];
    // debug(ans);
    
    vector<int> sum(2*maxc, 0), cnt(2*maxc, 0);
    for (int v = 2; v <= 2 * maxc; v *= 2) {
        for (int x : A) {
            sum[x % v] += x, ++cnt[x % v];
            int ix = (x % v ? v - x % v : 0);
            ans -= (x * cnt[ix] + sum[ix]) / v;
        }
        for (int x : A) sum[x % v] -= x, --cnt[x % v];
        // debug(v, ans);
    }
    print(ans);
}
