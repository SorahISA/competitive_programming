...
void solve() {
    ...
    auto binom = [&](int n, int m) -> int {
        return (n < m or m < 0 ? 0 : fact[n] * ifac[m] % mod * ifac[n-m] % mod);
    };
    
    int ans = 1, tot_step = 0;
    for (int i = 0, lst = 0; i < N; ++i) {
        if (i+1 == N or A[i+1] != A[lst]) {
            if (A[lst] != (lst & 1) or A[lst] != (i & 1)) { print(0); return; }
            for (int j = 1; j <= i-lst; j += 2) ans = (ans * j) % mod;
            tot_step += (i - lst) / 2;
            ans = ans * binom(tot_step, (i - lst) / 2) % mod;
            lst = i + 1;
        }
    }
    print(ans);
