...
void solve() {
    ...
    vector<int> lcs(N, K);
    deque<pii> deq{{lcs[0] = (A[0] == B[0]), 0}};
    while (SZ(deq)) {
        auto [_lcs, now] = deq[0]; deq.pf();
        if (_lcs > lcs[now]) continue;
        
        for (int x : adj[now]) {
            if (chmin(lcs[x], lcs[now] + (A[x] == B[lcs[now]]))) {
                A[x] == B[lcs[now]] ? deq.eb(lcs[x], x) : deq.ef(lcs[x], x);
            }
        }
    }
    cout << (lcs[N-1] == K ? "Yes" : "No") << "\n";
}
