...
void solve() {
    ...
    auto min_unknapsackable_a_with_remainder_r = [&](int r) -> int {
        int ans = -1;
        for (int m = 1; m < A[0]; ++m) {
            if (S < m*r) break;
            int diff = S - (dis[(S - m*r) % A[0]] + m*r);
            if (diff < 0) continue;
            // debug(r, m, diff, (diff / A[0] / m + 1) * A[0] + r);
            chmax(ans, (diff / A[0] / m + 1) * A[0] + r);
        }
        return ans;
    };
    ...
    while (true) {
        ...
        for (int r : available_remainders) {
            ...
            // }
            // debug(A, r, lo * A[0] + r);
            int a = min_unknapsackable_a_with_remainder_r(r);
            // debug(A, r, a);
            chmin(min_num, a);
        }
        if (min_num >= S) break;
