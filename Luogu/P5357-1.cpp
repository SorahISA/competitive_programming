...
struct ACautomaton {
    int cnt[sz], sub[sz][sigma], fail[sz], degI[sz], tok;
    ...
    void BuildFail() {
        ...
        while (!deq.empty()) {
            ...
            for (int i = 0; i < sigma; ++i) {
                if (sub[now][i]) fail[sub[now][i]] = sub[fail[now]][i], ++degI[sub[fail[now]][i]], deq.eb(sub[now][i]);
    ...
    void Query0(const string &str) {
        int now = 0;
        for (auto c : str) ++cnt[now = sub[now][c-'a']];
    }
    ...
    void Topo() {
        deque<int> deq;
        for (int i = 0; i <= tok; ++i) if (!degI[i]) deq.eb(i);
        while (!deq.empty()) {
            int now = deq.front(); deq.pf();
            if (fail[now]) {
                cnt[fail[now]] += cnt[now];
                if (!--degI[fail[now]]) deq.eb(fail[now]);
            }
        }
    }
...
void solve() {
    ...
    acam.Topo();
    for (auto &str : vec) cout << acam.Query1(str) << "\n";
}
