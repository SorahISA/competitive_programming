...
const int maxlen = 150 * 70 + 5;
const int maxsigma = 26;

struct AC {
    Vec<2, int> tr;
    vector<int> fail, cnt, app, vis, idx;
    int tok;
...
void solve(int N) {
    ...
    vector<string> vec(N);
    for (int id = 0; string &str : vec) cin >> str, ac.Insert(str, id++);
    ...
    int maxcnt = 0;
    for (int i = 0; i < N; ++i) chmax(maxcnt, ac.app[ac.idx[i]]);
    cout << maxcnt << "\n";
    for (int i = 0; i < N; ++i) {if (ac.app[ac.idx[i]] == maxcnt) cout << vec[i] << "\n";}
}
