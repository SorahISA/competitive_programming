...
void solve() {
    ...
    for (int i = 0; i < N; ++i) pre[S[i] - 'a'][i+1] = 1;
    for (int i = 0; i < 26; ++i) partial_sum(ALL(pre[i]), begin(pre[i]));
    ...
    for (int _ = 1; _ <= Q; ++_) {
        ...
        vector<int> cnt(26, 0); /// LHS - RHS
        for (int c = 0; c < 26; ++c) cnt[c] += (pre[c][M-1] - pre[c][L-1]) - (pre[c][R] - pre[c][M]);
        
        int non_zero = 0;
        for (int x : cnt) non_zero += !!x;
        if (non_zero == 0 or (non_zero == 2 and abs(cnt[S[M-1]-'a']) == 1)) ++ans;
    }
    cout << ans << "\n";
}
