...
void solve() {
    ...
    for (int _ = 1, x, y; _ <= Q; ++_) {
        ...
        if (answer.count(x*maxn + y)) {
            cout << answer[x*maxn + y] << " \n"[_ == Q];
            continue;
        }
        
        int cnt = 2 * (adj[x].count(y) ? adj[x][y] : 0);
        
        for (auto [to, cost] : adj[x]) {
            if (adj[y].count(to)) cnt += min(cost, adj[y][to]);
        }
        
        answer[x*maxn + y] = answer[y*maxn + x] = cnt;
        cout << cnt << " \n"[_ == Q];
    }
}
