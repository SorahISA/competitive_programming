...
void solve() {
    ...
    vector<vector<int>> life(R, vector<int>(C, 0));
    for (int i = 0; i < R; ++i) {
        for (int j = 0; j < C; ++j) {
            for (auto [dx, dy] : dir) life[i][j] += valid(i+dx, j+dy);
            if (board[i][j] == '#' or life[i][j] <= 1) que.eb(i, j), vis[i][j] = 1;
        }
    }
    
    while (SZ(que)) {
        // debug(que);
        auto [nx, ny] = que[0]; que.pf();
        if (board[nx][ny] == '^') return cout << "Impossible" << "\n", void();
        
        for (auto [dx, dy] : dir) {
            int cx = nx + dx, cy = ny + dy;
            if (valid(cx, cy) and !vis[cx][cy]) {
                if (--life[cx][cy] == 1) que.eb(cx, cy), vis[cx][cy] = 1;
            }
        }
    }
