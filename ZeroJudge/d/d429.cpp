...
void Tarjan(int now, int &tok_dfn) {
    ...
    for (int x : adj[now]) {
        if (!~dfn[x]) {
            ...
            if (low[x] >= dfn[now]) {
                bcc.eb(0);
                int p;
                while ((p = stk.back()) != x) bcc.back() |= 1 << p, stk.pb();
                bcc.back() |= 1 << p, stk.pb();
                bcc.back() |= 1 << now;
            }
        }
...
void solve() {
    ...
    for (int i = 0, tok_dfn = 0; i < N; ++i) {
        if (!~dfn[i]) {
            stk.clear(), Tarjan(i, tok_dfn);
            if (!SZ(adj[i])) bcc.eb(0), bcc.back() |= 1 << i;
        }
    }
    
    int cnt_bcc = SZ(bcc), ans = N;
    for (int mask = 0; mask < (1<<cnt_bcc); ++mask) {
        int have = 0;
        for (int i = 0; i < cnt_bcc; ++i) {
            if (mask >> i & 1) have |= bcc[i];
        }
        if (__builtin_popcount(have) == N) chmin(ans, __builtin_popcount(mask));
    }
    cout << ans << "\n";
}
