...
void Tarjan(int now, int fa, int &tok) {
    dfn[now] = low[now] = tok++;
    stk.eb(now);
    int flag_fa = true;
    for (int x : adj[now]) {
        if (x == fa and flag_fa) {flag_fa = false; continue;}
        if (!~dfn[x]) Tarjan(x, now, tok), chmin(low[now], low[x]);
        else chmin(low[now], dfn[x]);
    }
    if (fa == -1 or low[now] == dfn[now]) {
        ebcc.eb(0);
        int p;
        while ((p = stk.back()) != now) ebcc.back().eb(p), stk.pb();
        ebcc.back().eb(p), stk.pb();
    }
}
...
void solve() {
    ...
    for (int i = 0, tok = 0; i < N; ++i) {
        if (!~dfn[i]) Tarjan(i, -1, tok);
    }
    
    cout << SZ(ebcc) << "\n";
    for (vector<int> _vbcc : ebcc) {
        cout << SZ(_vbcc) << " ";
        for (int x : _vbcc) cout << x << " ";
        cout << "\n";
    }
}
