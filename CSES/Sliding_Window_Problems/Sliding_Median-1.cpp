...
void solve() {
    ...
    auto balance = [&]() -> void {
        while (abs(SZ(small) - SZ(large)) > 1) {
            if (SZ(small) > SZ(large)) {
                large.ee(*prev(end(small)));
                small.erase(prev(end(small)));
            }
            else {
                small.ee(*begin(large));
    ...
    auto add = [&](int x) -> void {
        if (SZ(large) and x >= *begin(large)) large.ee(x);
        else                                  small.ee(x);
        balance();
        // debug(small, large);
    };
    ...
    auto del = [&](int x) -> void {
        if (SZ(large) and x >= *begin(large)) large.erase(large.find(x));
