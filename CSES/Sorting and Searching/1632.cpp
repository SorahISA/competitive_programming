...
void solve() {
    ...
    vector<pii> movie(N);
    for (auto &[a, b] : movie) cin >> a >> b, --b;
    sort(ALL(movie), [](pii &p1, pii &p2) {return p1.Y < p2.Y;});
    
    int ans = 0;
    multiset<int> st;
    auto it = st.begin();
    for (int i = 0; i < K; ++i) st.ee(0);
    for (auto &[a, b] : movie) {
        if ((it = st.lower_bound(a)) == st.begin()) continue;
        st.erase(prev(it)), st.ee(b), ++ans;
    }
    cout << ans << "\n";
}
