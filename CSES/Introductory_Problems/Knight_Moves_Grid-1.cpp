...
void solve() {
    ...
    ans[0][0] = 0, ans[0][1] = 3, ans[0][2] = 2, ans[0][3] = 3;
    ans[1][0] = 3, ans[1][1] = 4, ans[1][2] = 1, ans[1][3] = 2;
    ans[2][0] = 2, ans[2][1] = 1, ans[2][2] = 4, ans[2][3] = 3;
    ans[3][0] = 3, ans[3][1] = 2, ans[3][2] = 3, ans[3][3] = 2;
    
    if (N == 4) ans[0][3] = ans[3][0] = 5;
    
    for (int i = 4; i < N; ++i) for (int j = 0; j <= i; ++j) {
        ans[i][j] = ans[j][i] = min({
            (i >= 1 and j >= 2 ? ans[i-1][j-2] : INF),
            (i >= 2 and j >= 1 ? ans[i-2][j-1] : INF),
            (i >= 2 and j+1 < N ? ans[i-2][j+1] : INF),
        }) + 1;
    }
