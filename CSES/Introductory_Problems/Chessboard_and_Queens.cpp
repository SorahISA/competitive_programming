...
void solve() {
    ...
    vector<int> used_c(8, 0), used_d(15, 0), used_a(15, 0);
    auto recur = [&](auto &&recur, int r, int &ans) -> void {
        if (r == 8) { ++ans; return; }
        for (int c = 0; c < 8; ++c) {
            if (used_c[c] or used_d[r-c+7] or used_a[r+c] or grid[r][c] == '*') continue;
            used_c[c] = used_d[r-c+7] = used_a[r+c] = 1;
            recur(recur, r+1, ans);
            used_c[c] = used_d[r-c+7] = used_a[r+c] = 0;
        }
    };
