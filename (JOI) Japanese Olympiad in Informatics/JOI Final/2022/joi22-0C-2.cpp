...
void solve() {
    ...
    vector<pii> AB(N);
    for (auto &[A, B] : AB) cin >> A >> B, B = B == -1 ? INF : B;
    ...
    auto calc = [&](int goal) -> double {
        double ret = INFd;
        vector<vector<double>> dp(N, vector<double>(goal+1, INFd));
        ...
        for (int i = goal; i <= K; ++i) {
            chmin(ret, dp[i-1][goal] + (double)suf_min_k[i][K-i] / (goal+1));
        }
        return ret;
    };
    ...
    while (lo < hi) {
        mi = (lo + hi) >> 1;
        if (calc(mi) < calc(mi+1)) hi = mi;
        else                       lo = mi + 1;
    }
    cout << fixed << setprecision(12) << min((double)suf_min_k[0][K], calc(lo)) << "\n";
}
