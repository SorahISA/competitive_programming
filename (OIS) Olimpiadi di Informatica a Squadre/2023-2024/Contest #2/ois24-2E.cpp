...
struct SegTree {
    ...
    void push(int now, int len) {
        ...
        tag[lt] += tag[now], seg[lt] += tag[now] * (len >> 1);
        tag[rt] += tag[now], seg[rt] += tag[now] * (len >> 1);
        tag[now] = 0;
    }
    ...
    void modify_range_add(int qL, int qR, int val, int now, int nL, int nR) {
        ...
        if (qR >  nM) modify_range_add(qL, qR, val, now<<1|1, nM+1, nR);
    ...
    int query_range_sum(int qL, int qR, int now, int nL, int nR) {
        ...
        if (qL <= nM) ret += query_range_sum(qL, qR, now<<1,   nL,   nM);
        if (qR >  nM) ret += query_range_sum(qL, qR, now<<1|1, nM+1, nR);
...
void solve() {
    ...
    vector<int> len(N); /// for each left end point
    for (int l = 0; l < N; ++l) {
        for (int i = 1; l+i-1 < N; ++i) {
            if (A[l+i-1] % i == 0) ++len[l];
            else break;
        }
    }
    ...
    for (int i = 0; i < Q; ++i) {
        ...
        queries.eb(l, r, i);
        // int ans = 0;
        // for (int i = l; i <= r; ++i) ans += min(len[i], r-i+1);
        // print(ans);
    }
    sort(RALL(queries));
    ...
    for (auto [l, r, id] : queries) {
        while (L > l) --L, seg.modify_range_add(L, L+len[L]-1, 1);
        ans[id] = seg.query_range_sum(l, r);
    }
