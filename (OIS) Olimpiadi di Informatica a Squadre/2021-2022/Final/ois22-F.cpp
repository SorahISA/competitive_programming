...
void solve() {
    ...
    vector<pii> paper(N);
    for (auto &[p, d] : paper) cin >> p >> d, d = (d == -1 ? L-1 : d);
    sort(ALL(paper), [](pii &p1, pii &p2) {return p1.Y > p2.Y;});
    
    int ans = 0;
    Prior<int> pq;
    for (int i = L-1, tok = 0; i >= 0; --i) {
        while (tok < N and paper[tok].Y == i) pq.ee(paper[tok++].X);
