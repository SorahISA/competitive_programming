...
void solve() {
    ...
    for (int i = 0; i < N; ++i) {
        cin >> vec[i];
        pre[i] = {vec[i] + N-i, vec[i] + N-i};
        suf[i] = {vec[i] + i, vec[i] + i};
        if (i) {
            chmin(pre[i].X, pre[i-1].X), chmax(pre[i].Y, pre[i-1].Y);
            chmin(suf[i].X, suf[i-1].X), chmax(suf[i].Y, suf[i-1].Y);
        }
    }
    
    int ans = 0;
    for (int i = 0; i < N; ++i) {
        chmax(ans, (vec[i] + N-i) - pre[i].X);
        chmax(ans, pre[i].Y - (vec[i] + N-i));
        chmax(ans, (vec[i] + i) - suf[i].X);
        chmax(ans, suf[i].Y - (vec[i] + i));
    }
    cout << ans << "\n";
}
