...
void solve() {
    int C = maxc, M = maxc, Y = maxc, K = maxc, c, m, y, k;
    
    for (int i : {0, 1, 2}) {
        cin >> c >> m >> y >> k;
        chmin(C, c), chmin(M, m), chmin(Y, y), chmin(K, k);
    }
    
    if (C + M + Y + K < maxc) cout << "IMPOSSIBLE" << "\n";
    else cout << C << " "
              << min(max(maxc-C, 0), M) << " "
              << min(max(maxc-C-M, 0), Y) << " "
              << min(max(maxc-C-M-Y, 0), K) << "\n";
}
