...
void solve() {
    ...
    fill(ALL(C), 0);
    for (int i = 0, tok = 1; i < N; ++i) {
        if (C[i]) continue;
        int now = i, flag = 0;
        while (!C[i]) C[now = A[now]] = tok, tok += !(flag = (flag + 1) % 5);
        if (flag == 1 and A[i] != i) --C[i];
        ++tok;
    }
    // debug(A, C);
}
...
int32_t main() {
    ...
    cin >> T >> N >> K, A.resize(N), C.assign(N, 1);
    for (int _ = 1; _ <= T; ++_) {
        state = 0;
        while (!state) solve(), query(), ++cnt;
        if (state == -1) return 0;
    }
    debug(cnt);
