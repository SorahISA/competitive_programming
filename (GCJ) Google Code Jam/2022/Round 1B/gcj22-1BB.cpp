...
void solve() {
    ...
    vector<array<int, 2>> order(N); /// (mn, mx)
    for (auto &[mn, mx] : order) {
        mn = INF, mx = -INF;
        for (int i = 0, x; i < P; ++i) cin >> x, chmin(mn, x), chmax(mx, x);
    }
    
    vector<array<int, 2>> dp(N, {0, 0}); /// (mn, mx)
    dp[0][0] = order[0][1] + (order[0][1] - order[0][0]), dp[0][1] = order[0][1];
    for (int i = 1; i < N; ++i) {
        auto &[ll, lr] = order[i-1];
        auto &[nl, nr] = order[i];
        dp[i][0] = min(dp[i-1][0] + abs(ll - nr) + abs(nr - nl),
                       dp[i-1][1] + abs(lr - nr) + abs(nr - nl));
        dp[i][1] = min(dp[i-1][0] + abs(ll - nl) + abs(nl - nr),
                       dp[i-1][1] + abs(lr - nl) + abs(nl - nr));
    }
